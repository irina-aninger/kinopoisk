...
function showFullInfo() {
    ...
    fetch(url)
        ...
        .then(output => {
            ...
            function fullVideo() {
                let video = 'https://api.themoviedb.org/3/movie/' + output.id + '/videos?api_key=3ac9e9c4b5b41ada30de1c0b1e488050&language=ru';

                fetch(video)
                    .then(function videoPlay() {
                        console.log(videoPlay);
                        let movieVideo = `
                            <div><video width="400" height="300" controls="controls" poster="video/duel.jpg"><source src="${output.key}" type='video/ogg; codecs="theora, vorbis"'></video></div>
                            </div>
                        `;

                        console.dir(videoPlay);

                        moviePage.innerHTML += movieVideo;
                    })
                    .catch(reason => {
                        movie.innerHTML = backMain + '<p class="error">Упс, что-то пошло не так!</p>';
                        console.error(reason || reason.status);
